...
The second command skips everything unless `QUENCH_LIVE_SMOKE=1` is set, but still
writes skip entries to `.smoke-results/`.

## Product Smoke Run

```powershell
$env:QUENCH_LIVE_SMOKE = "1"
uv run pytest smoke -n 0 -s --tb=short
```
...
`QUENCH_ALLOW_NO_PROVIDER_SMOKE=1`.
...
| `cli` | `quench-init`, server entrypoint, Claude CLI adaptive thinking, session cleanup | Claude CLI binary and provider only for real CLI |
...
| `voice` | generated WAV through local Whisper or NVIDIA NIM transcription | `VOICE_NOTE_ENABLED=true`, `QUENCH_SMOKE_RUN_VOICE=1` |

## Examples

```powershell
$env:QUENCH_LIVE_SMOKE = "1"
$env:QUENCH_SMOKE_PROVIDER_MATRIX = "open_router,nvidia_nim,deepseek,lmstudio,llamacpp,ollama"
uv run pytest smoke/product -n 0 -s --tb=short
```

```powershell
$env:QUENCH_LIVE_SMOKE = "1"
$env:QUENCH_SMOKE_TARGETS = "ollama"
...
```powershell
$env:QUENCH_LIVE_SMOKE = "1"
$env:QUENCH_SMOKE_TARGETS = "telegram,discord,voice"
$env:QUENCH_SMOKE_RUN_VOICE = "1"
uv run pytest smoke/product -n 0 -s --tb=short
```

```powershell
$env:QUENCH_LIVE_SMOKE = "1"
$env:QUENCH_SMOKE_TARGETS = "messaging,config,extensibility"
uv run pytest smoke/product -n 0 -s --tb=short
```

## Environment

- `QUENCH_ENV_FILE`: explicit dotenv path for startup/config scenarios.
- `QUENCH_LIVE_SMOKE=1`: enables live smoke execution.
- `QUENCH_ALLOW_NO_PROVIDER_SMOKE=1`: permits no-provider live smoke for harness work.
- `QUENCH_SMOKE_TARGETS`: comma-separated targets, or `all`.
- `QUENCH_SMOKE_PROVIDER_MATRIX`: comma-separated provider prefixes to require.
- `QUENCH_SMOKE_TIMEOUT_S`: per-request/subprocess timeout, default `45`.
- `QUENCH_SMOKE_CLAUDE_BIN`: Claude CLI executable name, default `claude`.
- `QUENCH_SMOKE_TELEGRAM_CHAT_ID`: Telegram chat/user ID for send/edit/delete.
- `QUENCH_SMOKE_DISCORD_CHANNEL_ID`: Discord channel ID for send/edit/delete.
- `QUENCH_SMOKE_INTERACTIVE=1`: enables manual inbound Telegram/Discord checks.
- `QUENCH_SMOKE_RUN_VOICE=1`: allows voice transcription backends to load/run.
...
provider in `QUENCH_SMOKE_PROVIDER_MATRIX`; selected-but-missing providers fail.
